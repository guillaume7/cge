...
func newCommand(startDir string, manager *repo.Manager, querier Querier) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:           "explain",
		Short:         "Explain repo-local graph retrieval results",
		SilenceErrors: true,
		SilenceUsage:  true,
		RunE: func(cmd *cobra.Command, _ []string) error {
			workspace, err := cmdsupport.RequireWorkspace(cmd, startDir, manager)
			if err != nil {
				return handleExplainError(cmd.OutOrStdout(), output, err)
			}
			...
			resultSet, err := querier.Query(cmd, workspace, input.Value)
			if err != nil {
				return handleExplainError(cmd.OutOrStdout(), output, err)
			}
			...
			response := resultEnvelope{
				...
				Index:       indexEnvelope{Status: resultSet.IndexStatus},
				Explanation: retrieval.BuildExplanation(input.Value, resultSet),
			}

			return cmdsupport.WriteSuccess(cmd.OutOrStdout(), output, "explain", response)
		},
	}

	cmd.Flags().StringVar(&task, "task", "", "Task text to explain")
...
type resultEnvelope struct {
	...
	Explanation retrieval.ExplanationSet `json:"explanation"`
}
...
func handleExplainError(w io.Writer, outputPath string, err error) error {
	if detail, ok := cmdsupport.ErrorDetailFromError(err); ok {
		return cmdsupport.WriteFailure(w, outputPath, "explain", detail, err)
	}

	var indexErr *textindex.Error
	if errors.As(err, &indexErr) {
		return cmdsupport.WriteFailure(w, outputPath, "explain", cmdsupport.ErrorDetail{
